...
export function Reading({
      ...
      return (
        <span
          class={classes(
            ...
            'tp:border-t-[1.5px] tp:border-x-0 tp:border-b-0'
          )}
        >
          {kana.ent}
        </span>
  ...
  const highLow = classes(
    ...
    'tp:border-b-0 tp:border-l-0'
  );
  const lowHigh = classes(
    'tp:border-r-(length:--border-width)',
    'tp:border-b-(length:--border-width)',
    'tp:border-l-0 tp:border-t-0'
  );
  const high = classes(
    'tp:border-t-(length:--border-width)',
    'tp:border-r-0 tp:border-b-0 tp:border-l-0'
  );
  const low = classes(
    'tp:border-b-(length:--border-width)',
    'tp:border-l-0 tp:border-t-0 tp:border-r-0'
  );
