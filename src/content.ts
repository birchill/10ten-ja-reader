...
import { removePuck, RikaiPuck, SafeAreaInsets } from './puck';
...
export class ContentHandler {
  ...
  showPopup(options?: { copyState?: CopyState; copyType?: CopyType }) {
    ...
    // TODO: Set this based on whether the most recent mousemove event was
    // generated by the puck or not, not simply on whether or not the puck is
    // available.
    let cursorClearance: SafeAreaInsets;
    if (this.puck) {
      const {
    ...
    const {
      ...
      // TODO: As above, this should be based on what generated the most recent
      // mousemove event, not simply on whether or not the puck is available.
      preferredSide: this.puck ? 'above' : 'below',
      safeArea,
    });
